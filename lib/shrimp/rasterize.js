...
if (system.args.length < 3 || system.args.length > 12) {
  ...
  console.log('[SHRIMP DEBUG] Address: ' + address)
  page.open(address, function (status) {
    console.log('[SHRIMP DEBUG] Status: ' + status)
    if (status !== 'success' || (statusCode != 200 && statusCode != null)) {
      ...
      console.log('[SHRIMP DEBUG] Rendering Time: ' + render_time);
      window.setTimeout(function () {
        ...
        console.log('[SHRIMP DEBUG] rendered to: ' + output, new Date().getTime());
        phantom.exit();
      }, render_time);
    }
  });
